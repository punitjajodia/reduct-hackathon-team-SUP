...
const CreateCampaign = ({challenges}) =>
{
    ...
    switch(currentStep) {
      case STEPS.CAMPAIGN_NAME:
        return(<>{heading}
        <h3>
          Enter campaign name</h3>
          <input value={campaignName} onChange={e => setCampaignName(e.target.value)}></input>
          <button onClick={() => setCurrentStep(STEPS.SELECT_CHALLENGES)}>Go to Next Step</button>
          </>);
      ...
      case STEPS.CREATE_LINK:
        ...
        {heading}<h3>
          Link Copied </h3>
          Following challenges are selected for your campaign. You can now use the generated link to send to your list of candidates. 
          </>);
      default:
